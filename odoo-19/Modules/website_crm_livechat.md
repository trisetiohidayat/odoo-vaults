...
- [Modules/website_crm](website_crm.md) — `website.visitor.lead_ids`, `crm.lead.visitor_ids`, Page Views stat button
- [Modules/website_livechat](website_livechat.md) — `discuss.channel.livechat_visitor_id`, livechat session tracking
- [Modules/crm_livechat](crm_livechat.md) — `/lead` command, chatbot "Create Lead" step type, `origin_channel_id` on lead
- [Modules/im_livechat](im_livechat.md) — Base livechat channel, operator routing, `discuss.channel` base model
- [Modules/im_livechat](im_livechat.md) — Chatbot script engine, step types, `_process_step` dispatch
- [Modules/CRM](CRM.md) — `crm.lead` base model, merge wizard, assignment
- [Core/Fields](Fields.md) — Many2many `(4, id)` link command, computed fields, `groups` attribute
- [Patterns/Workflow Patterns](Workflow Patterns.md) — Lead creation from non-CRM contexts
