...
- [Modules/Stock](Stock.md) — Stock module overview and quant/picking type documentation
- [Core/Fields](Fields.md) — Field type reference (Many2one, One2many, computed fields)
- [Core/API](API.md) — `@api.depends`, `@api.onchange`, `@api.constrains` decorators
- [Patterns/Security Patterns](Security Patterns.md) — ACL, ir.rule, and field-level security
- [Patterns/Workflow Patterns](Workflow Patterns.md) — State machine implementation in Odoo
- [Tools/ORM Operations](ORM Operations.md) — `search()`, `browse()`, `write()`, domain operators
