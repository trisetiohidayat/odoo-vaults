...
Bridges the [[Modules/hr_skills]] skills taxonomy with the [[Modules/hr_recruitment]] applicant pipeline. Allows recruiters to record skills and certifications on applicants, score applicants against job skill requirements, find matching candidates, and automatically provision `hr.employee.skill` records upon hiring.
...
| Model | Module | Role |
|---|---|---|
| `hr.skill` | [[Modules/hr_skills]] | Skill ontology (name, type, active) |
| `hr.skill.type` | [[Modules/hr_skills]] | Skill categories (Languages, Technical, Soft Skills, Certifications) |
| `hr.skill.level` | [[Modules/hr_skills]] | Proficiency levels with `level_progress` (integer 0-100) |
| `hr.job.skill` | [[Modules/hr_skills]] | Job required skills with required `level_progress` |
| `hr.applicant` | [[Modules/hr_recruitment]] | Applicant record (extended) |
| `hr.job` | [[Modules/hr_recruitment]] | Job position (extended) |
| `hr.recruitment.degree` | [[Modules/hr_recruitment]] | Education level with `score` (float) |
| `hr.talent.pool` | `hr_recruitment_extended` | Talent pool applicant (linked via `pool_applicant_id`) |
| `hr.employee.skill` | [[Modules/hr_skills]] | Employee skill (target when hiring) |
