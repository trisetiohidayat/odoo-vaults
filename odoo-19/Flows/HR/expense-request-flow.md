...
└─► Journal entry posted → state = 'done' (sheet) / 'posted' (move)
       └─► Payment triggered via [Flows/Account/payment-flow](payment-flow.md)
```
...
| Payment triggered | `account.payment` | Employee reimbursement via [Flows/Account/payment-flow](payment-flow.md) |
...
- [Flows/HR/employee-creation-flow](employee-creation-flow.md) — Employee setup (payable account, department)
- [Flows/Account/payment-flow](payment-flow.md) — Reimbursement payment after expense approval
- [Core/API](API.md) — @api.depends, @api.onchange decorator patterns
- [Patterns/Workflow Patterns](Workflow Patterns.md) — State machine pattern used in hr.expense.sheet
