...
|------|------|-------------|
| 🔀 Technical Flow | [Flows/Helpdesk/ticket-creation-flow](ticket-creation-flow.md) | Full method chain from ticket create to SLA application |
| 🔀 Technical Flow | [Flows/Helpdesk/ticket-resolution-flow](ticket-resolution-flow.md) | Ticket close, rating, reopen, and escalate |
| 📖 Module Reference | [Modules/Helpdesk](helpdesk.md) | Complete field and method reference |
| 🔧 Patterns | [Patterns/Workflow Patterns](Workflow Patterns.md) | State machine and workflow design |
...
> **⚡ System Trigger:** When `Email Alias` is enabled (`use_alias = True`), Odoo creates an incoming email gateway. Emails sent to `{alias_name}@{alias_domain}` automatically create `helpdesk.ticket` records. See [Flows/Helpdesk/ticket-creation-flow](ticket-creation-flow.md) for the `message_new()` flow.
...
| Type | Link | Description |
|------|------|-------------|
| 🔀 Technical Flow | [Flows/Helpdesk/ticket-creation-flow](ticket-creation-flow.md) | Full method chain — for developers |
| 🔀 Technical Flow | [Flows/Helpdesk/ticket-resolution-flow](ticket-resolution-flow.md) | Ticket close, rating, and reopen — for developers |
| 📖 Module Reference | [Modules/Helpdesk](helpdesk.md) | Complete field and method list |
