...
## Overview
Extension bridging [[Modules/l10n_in_purchase]] + [[Modules/l10n_in_stock]] + `purchase_stock`. Adds the warehouse address to vendor bills created from Purchase Orders — a legal requirement in Indian GST for determining the place of supply and validating the originating warehouse.
...
3. `l10n_in_purchase_stock` overrides `_l10n_in_get_warehouse_address()` (via `l10n_in_stock` base) to read warehouse address from PO and inject into the invoice (used in e-invoice JSON generation in [[Modules/l10n_in_edi]])
