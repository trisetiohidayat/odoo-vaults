...
- [[Modules/MRP]] — Base manufacturing module (workorders, MOs, BoMs)
- [[Modules/stock_account]] — Stock valuation accounting integration
- [[Modules/Account]] — General accounting (journal entries, accounts)
- [[Modules/Analytic]] — Analytic accounting (distribution, applicability)
- [[Core/API]] — ORM decorators used in this module (`@api.depends`, `@api.constrains`)
- [[Core/Fields]] — Field types: `Json` for analytic distribution, `Many2many` for relations
- [[Patterns/Workflow Patterns]] — MO state machine (`confirmed` -> `progress` -> `done`)
- [[Patterns/Security Patterns]] — ACL and record rules in multi-company environments
