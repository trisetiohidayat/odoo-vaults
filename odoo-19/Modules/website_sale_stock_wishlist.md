...
- [Modules/website_sale_stock](odoo-18/Modules/website_sale_stock.md) — Provides `_send_availability_email()` cron, `product.product` stock fields, and `stock_notification_partner_ids`
- [Modules/website_sale_wishlist](odoo-17/Modules/website_sale_wishlist.md) — Provides `product.wishlist` model, `/shop/wishlist` controller, and `AddProductToWishlistButton` interaction
- [Modules/website_sale](odoo-18/Modules/website_sale.md) — Core e-commerce; provides `VariantMixin`, `WebsiteSale` interaction, and combination info endpoint
- [Modules/Product](odoo-18/Modules/product.md) — Product variant model; all stock and wishlist operations ultimately target `product.product`
- [Modules/Stock](odoo-18/Modules/stock.md) — `stock.quant` for inventory tracking; `stock.location` for warehouse-specific stock levels
