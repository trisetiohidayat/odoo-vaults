...
The `account_fleet` module bridges the [Modules/Fleet](odoo-18/Modules/fleet.md) and [Core/Account](odoo-18/Modules/account.md) modules. It enables vendor bills (purchase invoices) to be linked to specific vehicles, automatically creates service log entries from bill lines, and provides a "Bills" button on the vehicle form for direct access to all accounting documents related to a vehicle.
