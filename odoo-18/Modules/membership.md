...
Manages organization membership subscriptions with invoice-based billing. Tracks membership lines linked to `account.move.line` invoices. Differs from [Modules/loyalty](loyalty.md) in that membership is about recurring paid subscriptions (clubs, associations, NGOs), not point-based reward programs.
