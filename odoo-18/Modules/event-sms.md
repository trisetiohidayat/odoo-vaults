...
Bridges the [Modules/Stock](odoo-18/Modules/stock.md) event scheduling engine with the SMS gateway. Adds `sms` as a third `notification_type` alongside `mail` on `event.mail` and `event.type.mail`, enabling SMS reminders and post-registration confirmations for event attendees.
...
- [Modules/Stock](odoo-18/Modules/stock.md) — event.sms notification context
