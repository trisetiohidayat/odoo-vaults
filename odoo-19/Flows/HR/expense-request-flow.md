...
└─► Journal entry posted → state = 'done' (sheet) / 'posted' (move)
       └─► Payment triggered via [[Flows/Account/payment-flow]]
```
...
| Payment triggered | `account.payment` | Employee reimbursement via [[Flows/Account/payment-flow]] |
...
- [[Flows/HR/employee-creation-flow]] — Employee setup (payable account, department)
- [[Flows/Account/payment-flow]] — Reimbursement payment after expense approval
- [[Core/API]] — @api.depends, @api.onchange decorator patterns
- [[Patterns/Workflow Patterns]] — State machine pattern used in hr.expense.sheet
