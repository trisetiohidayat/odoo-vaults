...
- [Modules/Event](event.md) — Core event module (event.event, event.registration, event.event.ticket, event.slot)
- [Modules/Website](website.md) — Website framework (website.menu, website.visitor, website.page)
- [Core/API](API.md) — `@api.depends`, `@api.depends_context`, computed fields with search
- [Core/Fields](Fields.md) — Field types, translate=True, store behavior, index='btree_not_null'
- [Patterns/Security Patterns](Security Patterns.md) — ir.rule, ACL CSV, record-level security
- [Patterns/Workflow Patterns](Workflow Patterns.md) — State machine and tracking patterns
