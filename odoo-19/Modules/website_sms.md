...
- [[Modules/website]] -- defines `website.visitor`, `website.track`, visitor tracking, and the `access_token` mechanism
- [[Modules/sms]] -- defines `sms.composer`, `sms.sms`, SMS templates, and the SMS gateway abstraction
- [[Modules/crm]] -- can extend `website.visitor` with CRM leads; `website_sms` is compatible with that extension via the `_check_for_sms_composer` hook
