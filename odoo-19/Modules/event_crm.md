...
`event_crm` bridges the [Modules/event](event.md) and [Modules/CRM](CRM.md) modules. When attendees register for events (or confirm attendance, or attend), the module can automatically create `crm.lead` records — typed either as `lead` or `opportunity` — pre-populated with contact information from the registration.
...
- [Modules/event](event.md) — Event management (registration, event, ticket models)
- [Modules/CRM](CRM.md) — CRM/leads/opportunities
