...
The `website_sms` module bridges the [Modules/website](odoo-18/Modules/website.md) visitor-tracking system with the [Modules/sms](odoo-18/Modules/sms.md) SMS sending infrastructure. It adds a **Send SMS** action button to `website.visitor` records in the Odoo backend UI (form, kanban, and list views), enabling backend users to contact a known website visitor via SMS -- but only when the visitor is linked to a `res.partner` record that carries a phone number.
...
- [Modules/website](odoo-18/Modules/website.md) -- defines `website.visitor`, `website.track`, visitor tracking, and the `access_token` mechanism
- [Modules/sms](odoo-18/Modules/sms.md) -- defines `sms.composer`, `sms.sms`, SMS templates, and the SMS gateway abstraction
- [Modules/crm](odoo-18/Modules/CRM.md) -- can extend `website.visitor` with CRM leads; `website_sms` is compatible with that extension via the `_check_for_sms_composer` hook
