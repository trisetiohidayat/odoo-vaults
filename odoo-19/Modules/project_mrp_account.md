...
- [[Modules/mrp_account]] — MO costing, WIP journal entries, work order analytic lines, `manufacturing_order` AAL category
- [[Modules/project_mrp]] — `project_id` on MO (from BoM), project stat buttons for MOs and BoMs
- [[Modules/Stock]] — Stock move costing and `_create_analytic_move()` from `stock_account`
- [[Patterns/Workflow Patterns]] — State machine pattern (MO workflow: draft → confirmed → done)
