...
- [[Modules/Stock]] — `stock.picking` base model, `button_validate` flow, `_pre_action_done_hook`, `_send_confirmation_email`
- [[Modules/SMS]] — `sms.template`, `_message_sms_with_template`, IAP SMS gateway
- [[Modules/res.partner]] — `partner_id.phone` field used for SMS recipient
- [[Core/API]] — `@api.model`, `@api.onchange` decorators relevant to `ResConfigSettings`
- [[Patterns/Security Patterns]] — ir.rule record rules, ACL CSV format, transient model security
