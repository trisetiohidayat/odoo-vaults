...
- [Modules/mail](mail.md) -- `mail.thread` notifications via `message_notify()`
- [Core/API](API.md) -- `@api.depends`, `@api.constrains`, `compute_sudo`, `@api.model`
- [Core/Fields](Fields.md) -- `Domain` field class, `Many2one`, `One2many`, `index='btree_not_null'`
- [Patterns/Workflow Patterns](Workflow Patterns.md) -- Cron-based automation, `state='code'` cron type
- [Patterns/Security Patterns](Security Patterns.md) -- ACL CSV, `sudo()`, record rules, `all_group_ids`
- [Tools/ORM Operations](ORM Operations.md) -- `search()`, `flush_all()`, `sudo()`, `exists()`, `read_group`, batch processing with `split_every`
