...
- [Modules/phone_validation](phone_validation.md) — Phone number formatting, E.164 normalization, `_phone_get_number_fields`, `_phone_format`
- [Modules/mail](mail.md) — `mail.thread`, `mail.message`, `mail.notification`, `mail.render.mixin`, `template.reset.mixin`
- [Modules/iap_mail](iap_mail.md) — IAP account infrastructure, credits management
- [Core/API](API.md) — `@api.model`, `@api.depends`, `@api.depends_context`, `@api.autovacuum`
- [Patterns/Workflow Patterns](Workflow Patterns.md) — State machine design (used by `sms.sms` state field)
- [Core/BaseModel](BaseModel.md) — `try_lock_for_update()`, `sudo()`, recordset operations
