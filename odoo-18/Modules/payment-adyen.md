...
- [Modules/payment](payment.md) — Base payment module (`payment.provider`, `payment.transaction`, `payment.token`)
- [Core/API](API.md) — `@api.depends`, `@api.onchange`, access token pattern
- [Patterns/Security Patterns](Security Patterns.md) — ir.rule, field groups, `base.group_system`
