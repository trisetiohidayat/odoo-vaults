...
- [Modules/sale](Sale.md) — Base sale module (reinvoice engine: `_sale_*` methods on `account.move.line`)
- [Modules/hr_expense](hr_expense.md) — Employee expense management (base `hr.expense` model)
- [Modules/sale_management](sale_management.md) — Sales management wrapper (depends chain)
- [Core/API](API.md) — `@api.depends`, `@api.onchange`, `@api.model` decorator patterns
- [Patterns/Workflow Patterns](Workflow Patterns.md) — State machine and action button patterns